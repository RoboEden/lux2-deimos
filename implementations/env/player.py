    ...
    def early_setup(self, step: int, obs, remainingOverageTime: int = 60):
        ...
        if step == 0:
            ...
            if factories_to_place > 0 and my_turn_to_place:
                ...
                # ice_sum = ndimage.convolve(obs['board']['ice'], kernal, mode='constant', cval=0.0)
                # ore_sum = ndimage.convolve(obs['board']['ore'], kernal, mode='constant', cval=0.0)
                # rubble_sum = ndimage.convolve(rubble_0, kernal, mode='constant', cval=0.0)
                # score = (ice_sum>0)*ice_weight + (ore_sum>0)*ore_weight + np.log(ice_sum + 1.0)*ice_log_weight +np.log(ore_sum+1.0) * ore_log_weight + rubble_sum * rubble_weight
                # score = score * valid_spawns_mask
                ...
                # score = np.log(ice_sum + 0.1) * np.log(ore_sum+np.e) + rubble_sum * rubble_weight
                # score = (score - np.min(score)) * valid_spawns_mask
